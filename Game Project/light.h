...
public:
   Light(const Triple& il = Triple(0.0,0.0,1.0),
      ...
      effectiveRad(sqrt(
        brightness/Gamedata::
        getInstance().getXmlFloat("TorchLight/noneffectiveLighting")))
      {}

   Light(std::string name) :
      loc(Triple(Gamedata::getInstance().getXmlFloat(name + "lightSpec/light" +"X"),
                 Gamedata::getInstance().getXmlFloat(name + "lightSpec/light" +"Y"),
                 Gamedata::getInstance().getXmlFloat(name + "lightSpec/light" +"Z"))),
      translated(),
      brightness(Gamedata::getInstance().getXmlFloat(name + "brightness")),
      effectiveRad(
        sqrt( brightness/Gamedata::getInstance().
              getXmlFloat(name + "noneffectiveLighting"))
      )
      {}
   ...
   void update(const Drawable* obj);
